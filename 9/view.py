...
def show_contacts(book: dict[int:dict[str,str]], message):
    if book:
        ...
        print('\n' + '='*(size_name + size_phone + size_comment + 9))
        ...
        print('='*(size_name + size_phone + size_comment + 9) + '\n')
    else:
...
def change_index(message: str) -> int:
    while True:
        index = input(message)
        return index


def add_change_contact() -> dict:
    changes = {}
    for key, value in text.contact_changes.items():
        changes[key] = input(value)
    return changes


def key_name(dictionary: dict[str,str]) -> str:
